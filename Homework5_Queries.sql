...
SELECT * FROM dbo.listRoomsByPrice(60,200);

--Command to undercut the cheapest room by a penny (or 1 gold in my schema):
DECLARE @price int;
DECLARE @tavern int;

SELECT @price = (SELECT TOP(1) Cost FROM dbo.listRoomsByPrice(60,200) - 1;
SELECT @tavern = (SELECT TOP(1) TavernID FROM dbo.listRoomsByPrice(60,200);
